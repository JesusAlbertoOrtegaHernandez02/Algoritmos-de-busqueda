...
def graph_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            countOpen += len(fringe)
            print("La cantidad de nodos visitados son:", len(closed) +1)
            print("La cantidad de nodos generados son:", countOpen)
            return node
        if node.state not in closed:
            closed[node.state] = True
            a = fringe.extend(node.expand(problem))
...
def branch_and_bound_graph_search(problem):
    ...
    return graph_search(problem, NewQueue())

def branch_and_bound_with_heuristic_graph_search(problem):
    """Search with branch and bound involving heuristic."""
