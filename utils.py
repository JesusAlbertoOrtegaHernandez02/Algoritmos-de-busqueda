...
class HeuristicQueue(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda node: node.path_cost + self.problema.h(node), reverse=True)
        

    def pop(self):
        # Coste de camino + heuristica(distancia recta entre el nodo y el goal)
        # Con heuristica: Aunque la distancia sea mas corta de un nodo a otro cuando le añades la heuristica
        # aumenta en comparación con otros nodos, de manera que se estime primero un camino que sea mas cercano
        # al nodo meta.
